#!/usr/bin/env python3
"""
Script to add PlaylistRepositoryTests.swift to the Xcode project
"""
...
# New file to add
filename = 'PlaylistRepositoryTests.swift'
...
# Add to IPTVPlayerTests group
tests_group_pattern = r"(A10000060 /\* IPTVPlayerTests \*/ = \{[^}]+children = \([^)]+)"
...
# Look for the IPTVPlayerTests target's sources build phase
test_sources_pattern = r"(/\* Sources \*/ = \{[^}]+buildActionMask[^}]+files = \([^)]*A[0-9A-F]{8} /\* .*Tests\.swift in Sources \*/,[^)]+)"
sources_ref = f"\t\t\t\t{build_id} /* {filename} in Sources */,"
content = re.sub(test_sources_pattern, r"\1\n" + sources_ref, content, count=1)
